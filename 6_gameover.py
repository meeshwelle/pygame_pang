...
while running:
    ...
    #공 위치 정의
    for ball_index, ball_val in enumerate(balls): #볼 리스트에 있는걸 하나씩 가져와서 인덱스와 그에 해당하는 밸류를 출력
        ...
        else: #그 외릐 모든 경우에는 속도를 증가 (시작값이 -니 속도가 점점 줄어듬 띠용 띠용 bounce할때처럼)
            ball_val["to_y"] += 0.5
    ...
    for ball_index, ball_val in enumerate(balls): #볼 리스트에 있는걸 하나씩 가져와서 인덱스와 그에 해당하는 밸류를 출력
        ...
        for weapon_index, weapon_val in enumerate(weapons):
            ...
            if weapon_rect.colliderect(ball_rect):
                ...
                if ball_img_index < 3: #i.e. 가장 작은 공이 아니면 나뉘어짐
                    #현재 공 크기 정보 가지고 옴
                    ball_width = ball_rect.size[0]
                    ball_height = ball_rect.size[1]

                    #나눠진 공 정보
                    small_ball_rect = ball_images[ball_img_index+1].get_rect() # +1 bc 한단계 작아진 공
                    ...
                    balls.append({
                        ...
                        "to_y" : -6, #공이 위로갔다 내려갔다
                        "init_speed_y" : ball_speed_y[ball_img_index+1] #y 최초 속도
                    })
        ...
        break #안쪽 for 문에서 break를 만나면 여기로 진입 가능. 2중 문을 한번에 탈출
    ...
    #충돌된 공/무기 없애기
    if ball_to_remove > -1:
        del balls[ball_to_remove] #which has the index
        ball_to_remove = -1
